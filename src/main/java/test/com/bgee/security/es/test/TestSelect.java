...
public class TestSelect {
    ...
    public void data(){
        ...
        for(int i=0; i<jsonArray.size(); i++){
            ...
            response = client.prepareIndex("film","dongzuo").setSource(json.toString(),XContentType.JSON).get();
            System.out.println("索引名称："+response.getIndex());
...
//    }
    ...
    public void testSearchParam(){
        SearchRequestBuilder srb = client.prepareSearch("film").setTypes("dongzuo");
        SearchResponse sr = srb.setQuery(QueryBuilders.matchQuery("title","星"))
                                .setFetchSource(new String[]{"title","price","publishDate"},null)
                                .execute()
                                .actionGet();
        SearchHits hits = sr.getHits();
